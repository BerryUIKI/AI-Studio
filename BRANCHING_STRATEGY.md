...
Configure repository branch protection for `main` and `dev`: no direct pushes, at least one independent review, required status checks, and up-to-date PR branches. Release and hotfix PRs also require product-owner release approval before publication. These are required governance settings; documentation alone does not prove GitHub enforcement is configured.
