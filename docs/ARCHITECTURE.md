...
- **Python Resolution Hierarchy**:
  1. `runtime/python/Scripts/python.exe` or `runtime/python/python.exe`: Bundled portable runtime (Zero-Python clean machine distribution, satisfying L01).
...
- **Engine Update & Rollback Semantics**:
  ...
  - The rollback restores repository code; it does not roll back pip wheel modifications inside the virtualenv. In the event of broken package dependencies after an interrupted update, recovery requires manual intervention, engine reinstallation, or restoring an external backup.
